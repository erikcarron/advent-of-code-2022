...
let findDirectoryToDelete = directorySizes => {
  let totalDiskSize = 70000000
  let usedDiskSize = directorySizes['/'].size
  let unusedDiskSize = totalDiskSize - usedDiskSize
  let requiredUnusedDiskSize = 30000000
  let freeUpSize = requiredUnusedDiskSize - unusedDiskSize

  return Object.keys(directorySizes).reduce((accumulator, currentValue) => {
    if (directorySizes[currentValue].size >= freeUpSize && directorySizes[currentValue].size < accumulator.size) {
      return { path: currentValue, size: directorySizes[currentValue].size }
    }

    return accumulator
  }, { path: '?', size: totalDiskSize })
}
...
let directoryToDelete = findDirectoryToDelete(directorySizes)
let answer = directoryToDelete.size
